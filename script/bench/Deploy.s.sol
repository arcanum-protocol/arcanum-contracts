...
import {toX96, toX32, toX16, sort, dynamic, updatePrice, AbstractFixedValueOracle} from "../../test/MultipoolUtils.t.sol";
...
contract Deploy is Script {
    function run() external {
        ...
        Multipool mpImpl = new Multipool{salt: keccak256(abi.encode("MultipoolSalt222222", 1))}();
        ERC1967Proxy proxy = new ERC1967Proxy{salt: keccak256(abi.encode("ProxySalt22222", 1))}(
            address(mpImpl),
            ""
        );
        ...
        mp.initialize("Name", "SYMBOL", address(stakerProxy), uint96(toX32(0.1e18)));
...
//   token 4  address:  0x2BeB75450683B0f6ac402387d392849949CE7A6D
        {
            ...
            for (uint i = 0; i < tokens.length; i++) {
                tokens[i] = new MockERC20WithDecimals{salt: keccak256(abi.encode("TokenSalt2222", "token", i))}("token", "token", decimals[i]);
