...
describe("MultipoolRouter", function() {
    let alice: any;
    let bob: any;
    let carol: any;
    let owner: any;
    ...
    beforeEach(async function() {
        ...
        await etf1.connect(owner).updateAssetPercents(
            assets[0].address,
            toDecimal(40),
        );
        await etf1.connect(owner).updateAssetPercents(
        ...
        await etf1.connect(owner).setHalfDeviationFeeRatio(toDecimal(3, 14));
        await etf1.connect(owner).setDeviationPercentLimit(toDecimal(1, 17));

        await etf1.connect(owner).setBaseTradeFee(toDecimal(5, 13));
        await etf1.connect(owner).setBaseBurnFee(toDecimal(1, 14));
        await etf1.connect(owner).setBaseMintFee(toDecimal(1, 14));
    ...
    it("Ops return refund for not used amount to user", async function() {
        ...
        expect(await assets[1].balanceOf(alice.address)).to.be.closeTo(
            balanceBefore,
            toDecimal(300),
        );
    ...
    it("Estimates works 2", async function() {
        const amountIn = toDecimal(1000000);
        await assets[0].connect(alice).approve(router.address, amountIn);
        // initial mint
        let result = await router.connect(alice).mintWithSharesOut(
            etf.address,
            assets[0].address,
            toDecimal(1000000),
            toDecimal(1000000),
