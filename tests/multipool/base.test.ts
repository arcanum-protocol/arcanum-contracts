...
describe("Multipool", function() {
    ...
    it("ETF mint and swap should work", async function() {
        ...
        const result = await etf.connect(alice).swap(assets[1].address, assets[0].address, toDecimal(10001, 14), alice.address);

        await expect(result).to.changeTokenBalances(assets[0], [etf.address, alice.address], ['-1100064997750112494', '1100064997750112494']);
    ...
    it("Mint more than transfer", async function() {
        ...
        // initial mint
        await etf.connect(alice).mint(assets[0].address, toDecimal(11), alice.address);

        await assets[0].connect(alice).transfer(etf.address, toDecimal(10));
        await expect(etf.connect(alice).mint(assets[0].address, toDecimal(11), alice.address)).to.be.revertedWith("MULTIPOOL: mint amount in exeeded");
    })
    ...
    it("Burn return unused transfer", async function() {
        ...
        expect((await etf.assets(assets[0].address)).quantity).to.eq(toDecimal(1000));
        ...
        await etf.connect(alice).burn(assets[0].address, toDecimal(10), alice.address);
        expect((await etf.assets(assets[0].address)).quantity).to.eq(toDecimal(990));
    ...
    it("Burn more than transfer", async function() {
        ...
        expect(await etf.balanceOf(alice.address)).to.eq(toDecimal(990));
        await expect(etf.connect(alice).burn(assets[0].address, toDecimal(15), alice.address)).to.be.revertedWith("ERC20: burn amount exceeds balance");
    })
    ...
    it("Burn zero amount doesn't affect balance", async function() {
        ...
        await etf.connect(alice).transfer(etf.address, toDecimal(10));
        expect(await etf.balanceOf(alice.address)).to.eq(toDecimal(990));
        await etf.connect(alice).burn(assets[0].address, toDecimal(0), alice.address);
    ...
    it("Ops in curve applies cashback or fee", async function() {
        ...
        //mint with fee 
        await assets[0].connect(alice).transfer(etf.address, toDecimal(10));
        await etf.connect(alice).mint(assets[0].address, toDecimal(9), alice.address);
        expect(await assets[0].balanceOf(alice.address)).to.eq("9999980999100000000000000");
        ...
        expect(await assets[1].balanceOf(bob.address)).to.eq("9999975997600000000000000");
        ...
        expect(await assets[1].balanceOf(bob.address)).to.eq("9999979997200039996000399");

        await etf.connect(owner).updateAssetPercents(assets[0].address, toDecimal(30));
        await etf.connect(owner).updateAssetPercents(assets[1].address, toDecimal(70));
        ...
        expect(await assets[1].balanceOf(bob.address)).to.eq("9999949994200039996000400");
    ...
    it("Swap can be performed without any part of etf", async function() {
        ...
        expect(await assets[0].balanceOf(carol.address)).to.eq(assetABefore.add("94999560606060606181"))
        expect(await assets[1].balanceOf(carol.address)).to.eq(assetBBefore.add("5011863382121378623"))
    })
    ...
    it("Update price", async function() {
        ...
        await etf.connect(alice).mint(assets[0].address, toDecimal(29), alice.address);
        // returned transfered tokens
        expect(await etf.balanceOf(alice.address)).to.eq(toDecimal(1029));
        // amount of refund
        expect(await assets[0].balanceOf(alice.address)).to.eq(assetBefore.sub(toDecimal(30)).add("997100000000000000"));
    })
