...
describe("Multipool contract maths", function() {
    ...
    it("Change fees and cure params", async function() {
        ...
        await etf.connect(owner).setBaseMintFee(toDecimal(1, 17));
    ...
    it("Change depeg base fee", async function() {
        let executionResult;
        let asset1: MpAsset;
        let asset2: MpAsset;
        ...
        await etf.connect(owner).setDepegBaseFeeRatio(toDecimal(5, 17)); //50% cashback goes to base fee
        ...
        expect(asset1.collectedFees).to.be.equal(
            toDecimal(4).add(BigNumber.from(BigInt("99999999999999997800") / BigInt(2))),
        );
        expect(asset1.quantity).to.be.equal(
            toDecimal(240000),
        );
        expect(asset1.collectedCashbacks).to.be.equal(
            BigNumber.from(BigInt("99999999999999997800") / BigInt(2)),
        );
        ...
        asset2Balance = await assets[1].balanceOf(etf.address);
        asset2 = await etf.assets(
            assets[1].address,
        );
        expect(asset2.collectedFees).to.be.equal(
            BigNumber.from("21984914648670107184").add(BigNumber.from(BigInt("148868598650258038906") / BigInt(2))),
        );
        expect(asset2.quantity).to.be.equal(
            "180000000000000000000000",
        );
        expect(asset2.collectedCashbacks).to.be.equal(
            BigNumber.from(BigInt("148868598650258038906") / BigInt(2)),
        );
