...
contract MultipoolRouterCases is Test {
        ...
        {
            {
                (uint amount, uint refund) =
                    router.mintWithSharesOut(address(mp), address(tokens[0]), 2e18, 0.8008e18, users[0], 0);
        ...
        {
            {
                (uint shares, uint refund) =
                    router.mintWithAmountIn(address(mp), address(tokens[0]), 0.8008e18, 2e18, users[0], 0);
    ...
    function test_Router_Burn_LowerThanTarget_DeviationInRange_Cashback() public {
...

        {
            {
                (uint amount, uint refund) =
                    router.burnWithSharesIn(address(mp), address(tokens[0]), 2e18, 0.724215971548658261e18, users[0], 0);
        ...
        {
            {
                (uint shares, uint refund) = router.burnWithAmountOut(
                    address(mp), address(tokens[0]), 0.724215971548658261e18, 2e18, users[0], 0
                );
        ...
        {
            {
                (uint amountOut, uint refundIn, uint refundOut) = router.swapWithAmountIn(
                    address(mp), address(tokens[1]), address(tokens[0]), 0.606e18, 1.188118811881188117e18, users[0], 0
                );
        ...
        {
            {
                (uint amountIn, uint refundIn, uint refundOut) = router.swapWithAmountOut(
                    address(mp), address(tokens[1]), address(tokens[0]), 1.188118811881188119e18, 0.606e18, users[0], 0
                );
