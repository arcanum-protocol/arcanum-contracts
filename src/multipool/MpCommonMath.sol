...
using {
    MpCommonMath.evalMint,
    MpCommonMath.evalBurn
    ...
    function evalMint(
        ...
        uint shareNew = ((asset.quantity + utilisableQuantity) * asset.price) /
            (context.usdCap +
                (utilisableQuantity * asset.price) /
                DENOMINATOR);
        uint targetShare = (asset.share * DENOMINATOR) /
            context.totalTargetShares;
        ...
        if (deviationNew <= deviationOld) {
            ...
            require(
                deviationNew < context.deviationLimit,
                "MULTIPOOL: deviation overflow"
            );
            uint depegFee = (context.halfDeviationFee *
                deviationNew *
                utilisableQuantity) /
                context.deviationLimit /
                (context.deviationLimit - deviationNew);
            uint deviationBaseFee = (context.depegBaseFee * depegFee) /
                DENOMINATOR;
    ...
    function evalBurn(
        ...
        if (
            ...
            if (deviationNew <= deviationOld) {
                ...
                uint feeRatio = (context.halfDeviationFee *
                    deviationNew *
                    DENOMINATOR) /
                    context.deviationLimit /
                    (context.deviationLimit - deviationNew);
