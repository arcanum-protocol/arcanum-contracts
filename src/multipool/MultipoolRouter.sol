...
contract MultipoolRouter {
    ...
    function burnWithSharesIn(
        ...
        (uint amountOut, uint _refund) = Multipool(poolAddress).burn(
            assetAddress,
            sharesIn,
            to
        );
    ...
    function swap(
        ...
        // No need to check sleepage because contract will fail if there is no
        // enough funst been transfered
        (uint amountIn, uint amountOut, , ) = Multipool(poolAddress).swap(
            assetInAddress,
            assetOutAddress,
            shares,
            to
    ...
    function swapWithAmountOut(
        ...
        (uint _amountIn,, uint _refundIn ,uint _refundOut ) = Multipool(poolAddress).swap(
            assetInAddress,
            assetOutAddress,
            shares,
            to
    ...
    function estimateMintSharesOut(
        ...
        address assetAddress,
        uint amountIn
    ) public view returns (uint sharesOut, uint fee, uint cashbackIn) {
        MpAsset memory asset = Multipool(poolAddress).getAssets(assetAddress);
        MpContext memory context = Multipool(poolAddress).getMintContext();
        uint totalSupply = Multipool(poolAddress).totalSupply();
        uint oldUsdCap = context.usdCap;
        ...
        sharesOut =
            (amountOut * asset.price * totalSupply) / DENOMINATOR / oldUsdCap;
    ...
    function estimateMintAmountIn(
        ...
        address assetAddress,
        uint sharesOut
    ) public view returns (uint amountIn, uint fee, uint cashbackIn) {
        MpAsset memory asset = Multipool(poolAddress).getAssets(assetAddress);
        MpContext memory context = Multipool(poolAddress).getMintContext();
        ...
        uint _amountIn = (sharesOut * oldUsdCap) * DENOMINATOR /
            asset.price /
            totalSupply;
        ...
        uint noFeeShareOut = (amountIn * asset.price * (totalSupply)) /
            oldUsdCap / DENOMINATOR;
        fee = noFeeShareOut * DENOMINATOR / sharesOut - 1e18;
    }
        ...
        returns (uint amountOut, uint fee, uint cashbackOut)
    {
        ...
        uint amountIn = (sharesIn * oldUsdCap * DENOMINATOR) /
            asset.price /
            totalSupply;
        ...
        uint noFeeSharesIn = (amountOut * asset.price * (totalSupply)) /
            oldUsdCap / DENOMINATOR;
    ...
    function estimateBurnSharesIn(
        ...
        address assetAddress,
        uint amountOut
    ) public view returns (uint sharesIn, uint fee, uint cashbackOut) {
        MpAsset memory asset = Multipool(poolAddress).getAssets(assetAddress);
        MpContext memory context = Multipool(poolAddress).getBurnContext();
        ...
        sharesIn =
            (amountIn * asset.price * totalSupply) / DENOMINATOR /
            oldUsdCap;
        ...
        )
    {
        ...
        shares =
            (mintAmountOut * assetIn.price * totalSupply) / DENOMINATOR /
            oldUsdCap;

        uint burnAmountIn = (shares * context.usdCap) * DENOMINATOR /
            assetOut.price /
            (totalSupply + shares);
