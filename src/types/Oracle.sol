...
function unpackWithdrawRequest(bytes32 packedWithdrawRequest) pure returns(WithdrawRequest memory withdrawRequest) {
    withdrawRequest.amount = getBits(packedWithdrawRequest, 0, 128);
    withdrawRequest.timestamp = getBits(packedWithdrawRequest, 128, 64);
}

function packWithdrawRequest(WithdrawRequest memory withdrawRequest) pure returns(bytes32 packedWithdrawRequest) {
    packedWithdrawRequest = setBits(packedWithdrawRequest, bytes32(uint(uint128(withdrawRequest.amount))), 0, 128);
    packedWithdrawRequest = setBits(packedWithdrawRequest, bytes32(uint(uint64(withdrawRequest.timestamp))), 128, 127);
}
...
function packOracleData(OracleData memory oracleData) pure returns(bytes32 packedOracleData) {
    ...
    packedOracleData = setBits(packedOracleData, bytes32(uint(uint128(oracleData.totalShares))), 128, 127);
    packedOracleData = setBits(packedOracleData, bytes32(oracleData.enabled == true ? uint(1) : 0), 191, 1);
}
