...
library FarmingMath {
    ...
    function updateRewards(PoolInfo memory pool, UserInfo memory user, uint currentBlockNumber) internal pure {
        updatePool(pool, currentBlockNumber);
        uint amount = user.amount * pool.arps / 1e18 - user.rd;
        user.accRewards += amount;
    }

    function deposit(PoolInfo memory pool, UserInfo memory user, uint currentBlockNumber, uint depositAmount) internal pure {
    ...
    function withdraw(PoolInfo memory pool, UserInfo memory user, uint currentBlockNumber, uint withdrawAmount) internal pure {
        updateRewards(pool, user, currentBlockNumber); 
    ...
    function updateDistribution(PoolInfo memory pool, uint currentBlockNumber, int rewardsDelta, uint newRpb) internal pure {
        updatePool(pool, currentBlockNumber);
        if (rewardsDelta >= 0) {
